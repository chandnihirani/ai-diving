## Diving progression – planning notes

### Current & projected dive profile

- **Current state (Feb 2025)**  
  - 9 dives completed (Indonesia – OW qualifying, Turkey, Antigua)  
  - Certification: **Open Water**  
  - Experience: mostly warm-water, reef-focused dives

- **Kenya trip (next month, March 2025)**  
  - Goal: complete **Advanced Open Water (AOW)**  
  - Expected additional dives: **5–6**  
  - After Kenya (projected April 2025 state):  
    - ~14–15 total dives  
    - Certifications: **OW + AOW**  
    - Locations: Indonesia, Turkey, Antigua, **Kenya**  

- **Medium-term direction (to end of 2027)**  
  - Focus on becoming an **objectively better, more versatile diver**, not hitting an arbitrary dive count.  
  - Priorities:  
    - Gradually **level up core skills** (buoyancy, trim, navigation, air management, situational awareness).  
    - Experience **different environments** (reefs, walls, wrecks, caverns, mild current, cooler water, night/low‑viz).  
    - Build a sustainable **cadence of 2–3 dive trips per year**, mostly **Europe/Med** with the flexibility to add **1 further‑afield trip** when it feels right.  
  - Logged dives are a **by‑product** of variety and progression, not the main goal.

---

### Trip sequencing idea

- **March 2025 – Kenya**  
  - Complete AOW and broaden skills beyond “just warm reef fun dives”.  
  - Adds **5–6 dives** → total ~14–15 dives.  

- **16–19 May 2025 – Mainland Sicily (coast)**  
  - **Your base: Syracuse / Ortigia** (south‑eastern Sicily).  
  - Plan for **one day of diving** from Syracuse (typically 2 dives, one morning or half‑day), or more if you add a day trip north.  
  - Typical mid‑May sea temperature: **~18–19°C** → comfortable with **5–7 mm rental wetsuit + hood** (widely available at local dive centers).  

  - **Syracuse / Ortigia – one day of diving (local)**  
    - **Where is Plemmirio?** The **Plemmirio Marine Reserve** is on the **Maddalena Peninsula**, a few kilometres **south of Syracuse** (Ortigia). It’s the same coast: the peninsula sticks out south from the city, with ~14 km of coastline and protected sea. Dive centres are in Siracusa / Santa Panagia; they take you by boat to the reserve (short ride). No need to change accommodation.  
    - **What the dive sites look like:**  
      - **Green vs red route (what it means):** At these sites the centre often offers two **routes** through the same dive – same place, different depth/length. **Green route** = shallower, shorter profile (max ~20 m); **red route** = deeper, more of the site (max ~26–28 m). You’re not choosing a different site – you’re choosing how deep/long you go. After AOW, the red route is a good way to use your deep training.  
      - **The Arches (Gli Archi)** – Wall dive, all levels. You drop to ~12 m onto a white rocky plateau with seaweed, then move into a circular hollow with **two large natural arches**. Under the smaller arch: crevices with cardinalfish; at ~17 m a big rock with groupers and brown meagre. A **steep canal** drops to ~28 m with cracked walls full of life; at ~18 m there’s a **bronze mermaid statue** (tribute to free diver Rossana Maiorca). Under the main arch, light through holes in the roof; walls covered in **colourful sponges and yellow anemones**; on the sand, flying gurnards and stingrays; shoals of bream. Depths: green route ~20 m, red route ~26 m. Viz usually good/excellent.
      - **The Cheese Cave (Grotte del Formaggio)** – Wall/cavern dive, all levels. **Large underwater caverns**, no confusing forks, solid rock. A **tunnel** from ~9–10 m down to ~18–19 m, walls covered in **sponges, bryozoa, yellow cluster anemones**; slipper lobster and bream. One cave has a **big opening (~20 m wide)** with a hole in the roof so **sunlight beams in** (classic “cathedral light” effect). The namesake cave has **two entrances at 90°** (L‑shape), so **blue light from two directions** and walls lined with white sponges; brown meagre around a big rock at ~10–12 m. Depths: green route ~20 m, red route ~28 m. Can get **medium** difficulty if there’s current.
      - **General:** Protected since 2004; clear water, reefs, walls, cracks and canyons; typical Med life (groupers, breams, amberjack, barracuda, morays, sponges, anemones). Some areas have archaeological interest (ancient and WWII); one spot has old motorbike/car wrecks (“le moto”).  
    - **Dive centres to contact (Siracusa area):**  
      - [Capo Murro Diving Center](https://www.capomurrodivingcenter.it/en/) – PADI resort in Plemmirio; guided dives, courses; sites include **The Arches** (rock formations, marine life, bronze mermaid memorial), plus other Plemmirio sites.  
      - [Anemone Diving Center](https://divingaway.com/en/dive-center-2565/anemone-diving-center) – Viale Santa Panagia, Siracusa; Plemmirio by boat; sites e.g. **Grotte del Formaggio**, **Grotta del Capo**, **Giant's Tongue**; caves, groupers, barracudas, breams; fun dives from ~€35.  
      - [Ortigia Diving](https://www.ortigiadiving.com/) – **Right in Ortigia** (Riva della Posta 5); immersioni al Plemmirio; guided dives, courses, equipment rental, boat to sites; very convenient if you’re staying on the island.  
      - *Note:* “ASD” in Italy often means *Associazione Sportiva Dilettantistica* (club legal name). In Syracuse, “ASD Diving” in search results may refer to **Anemone** (A.S.D. Anemone Diving Center) or another club – same Plemmirio area; if you have a specific link, we can double‑check.  
    - **When emailing:** ask for a **two‑tank morning** (or half‑day), mention **AOW** (post‑Kenya) and that you’d like **reef + cavern/grotto** if conditions allow.  
    - **Gear:** request **5–7 mm full wetsuit + hood** for May.  

    - **Why Plemmirio is a good fit after AOW (vs Kenya)**  
      - **Different environment:** Kenya = tropical Indian Ocean reef, warm water. Plemmirio = **temperate Mediterranean**: rocky walls, natural arches, **caverns with overhead** (no confusing maze, but proper “inside” feeling and light effects), **cooler water** (~18–19°C) and **5–7 mm wetsuit + hood** – so new buoyancy/trim and thermal experience. Fits your goal to try “caverns, cooler water”.  
      - **Different wildlife:** No tropical corals or East African reef fish. Here you get **Mediterranean** life: groupers, breams, brown meagre, morays, sponges and anemones (colour on the walls), cardinalfish in cracks, slipper lobster, flying gurnards, stingrays, shoals of bream; sometimes amberjack/barracuda. Different colours and behaviours to observe.  
      - **Gentle skill stretch (not extreme):** Sites are “all levels” and “low difficulty” (medium when current). Good **first post‑AOW** step: (1) **Cavern** – if you’ve done any short tunnel or rock pass‑through before (e.g. your log notes “tunnel of rocks like mini cave” at Black’s Point in Antigua – only if that rings a bell), Cheese Cave is a step up (larger caverns, cathedral light, no maze); if not, it’s simply your first proper cavern. (2) **Depth** – red routes to ~26–28 m; you’ll have done deep in Kenya, so this **consolidates** 20–30 m. (3) **Cooler water + wetsuit** – new for you; good time to get used to 5–7 mm before Malta. (4) **Current** – possible; if it’s on, it’s a mild introduction. To lean into the challenge, ask the centre for the **deeper (red) routes** and say you’re AOW and comfortable to 28 m.  

  - **Optional day trip north (Taormina / Catania area)** – only if you want a second day of diving or different sites:  
    - **Taormina / Giardini Naxos area (east coast)** – ~1.5–2 h drive from Syracuse.  
      - `Grotta Azzurra (Blue Grotto)`: cavern-style dive with beautiful light.  
      - `Isola Bella marine reserve`: walls with sponges, typical Med life (groupers, morays, octopus).  
      - `Capo Taormina / Roman columns`: easy archaeology-style dive with ancient ruins.  
      - `Blue Cave` / `Blue Grotto` variants around Capo Taormina (ask for a cavern/light‑effects dive).  
      - `Air Cave` / `Air Cavern` and similar small swim‑throughs (easy overhead environments).  
    - **When emailing Taormina centres, example sites to request**  
      - “Grotta Azzurra / Blue Grotto”  
      - “Isola Bella marine reserve walls”  
      - “Capo Taormina with Roman columns / archaeology site”  
      - “One cavern/cave‑style dive such as Blue Cave or Air Cave, if conditions allow”  
      - **Dive centres to contact**:  
        - [Blue Sea Diving Center](https://blueseadiving.it/) – [Maps](https://www.google.com/maps/search/?api=1&query=Blue+Sea+Diving+Center+Taormina+Mazzaro)  
        - [Sea Spirit Diving Resort](https://www.seaspirit.it/en/) – [Maps](https://www.google.com/maps/search/?api=1&query=Sea+Spirit+Diving+Resort+Giardini+Naxos)  
        - [Taormina Diving Center](https://www.divingtaormina.it/?lang=en) – [Maps](https://www.google.com/maps/search/?api=1&query=Taormina+Diving+Center+Giardini+Naxos)  
    - **Aci Trezza / Cyclops Islands marine area (near Catania)**  
      - Volcanic rock formations, swim‑throughs, and good chance of schooling fish and morays.  
    - **Cefalù (north coast)**  
      - Rocky reefs and walls; less “famous” by name, but solid Med reef diving via local centers.  

- **Early July 2025 – Europe trip (SECOND trip post-AOW)**  
  - Destination: **Malta**  
    - Plan for **6–8 dives** over the trip.  
    - Good match for a fresh AOW diver building on Kenya + Sicily experience.  
    - Introduces **wrecks, arches/swim‑throughs and caverns** with manageable conditions.  
    - Nice progression from Kenya reefs → Sicily coastal sites → Malta wrecks/caves.  
    - **Example sites to mention when emailing (so centres know what you’re aiming for)**  
      - **Cirkewwa area (northwest Malta)**  
        - `Rozi` tugboat wreck (≈30–35 m; classic AOW/deep profile).  
        - `P29` patrol boat wreck (similar depth range, often paired with Rozi/Cirkewwa reef).  
        - `Cirkewwa Arch` / `Right Arch` – natural arch / swim‑through (the “hole” you may have seen in photos).  
        - Cirkewwa reef & wall / plateau, with drop‑offs and smaller swim‑throughs.  
      - **Paradise Bay / adjacent reefs** – shallower, scenic reef dives; good balance day with less depth stress.  
      - **Stretch/conditions‑dependent options (only if the centre says appropriate for your experience + weather)**  
        - `Um El Faroud` wreck (deep, iconic but more advanced – only if they’re happy with your profile).  
        - Optional Gozo day‑trip to `Blue Hole` at Dwejra (famous vertical chimney + arch swim‑through) if they run it and conditions are good.  
    - **Areas & dive centres to consider (Malta north/east coast)**:  
      - **St Paul’s Bay / Bugibba area** – good base for shore and boat access to classic Malta wrecks and reefs.  
        - [Buddies Dive Cove](https://buddiesmalta.com/) – [Maps](https://www.google.com/maps/search/?api=1&query=Buddies+Dive+Cove+Bugibba+Malta)  
        - [Maltaqua](https://www.maltaqua.com/) – [Maps](https://www.google.com/maps/search/?api=1&query=Maltaqua+St+Paul%27s+Bay+Malta)  
        - [DiveBase Malta](https://divebasemalta.com/) – [Maps](https://www.google.com/maps/search/?api=1&query=DiveBase+Malta+Bugibba)  

- **Mid–end October 2025 – Europe / Mediterranean trip**  
  - Aim for **another Med destination (not Malta)**, e.g.:  
    - **Cyprus** (Larnaca/Protaras – warmest late‑season Med, famous wrecks like Zenobia).  
    - **Southern Greece** (Crete, Rhodes) or **southern Italy/Sicily**.  
    - **Southern Spain** (e.g. Costa del Sol / Almería) depending on conditions.  
  - Plan for **4–6 dives**.  
  - Water will be cooler but manageable with **5–7 mm rental wetsuits + hood** (you’re happy using whatever thickness shops provide).  

- **Late 2025 / early 2026**  
  - Adjust next destinations based on what you enjoy most from Malta + October Med (reefs vs wrecks vs “adventure” profiles).  

- **2026–2027**  
  - Keep a loose rhythm of **2–3 trips per year**, adjusting for budget/time/energy.  
  - Choose trips for **variety and progression** (new locations, new conditions, new types of dives) rather than chasing a specific total‑dives number.

---

### What the agent needs to handle

- **Dynamic timeline & state**
  - Track **current state**: certifications, total dives, locations, recent conditions.  
  - Track **planned trips** (e.g. Kenya in March) and expected impact (new certs, projected dive count).  
  - Maintain a **projected state** post‑trip (e.g. post-Kenya: AOW + ~18 dives).  

- **Planning logic**
  - Use current + projected capabilities to suggest **appropriate next destinations**  
    (e.g. post-Kenya AOW → Malta with wrecks/caves).  
  - Plan sequences like: **Kenya → Europe summer → late‑year trip**, not just one‑off holidays.  
  - Balance **goals** (50 dives, skills progression) with constraints (budget, time, seasonality).

- **Future-facing features**
  - Keep a notion of “**upcoming trip**” (Kenya) and “**next decision point**” (after Kenya).  
  - After each trip, update:  
    - Total dives  
    - Certs/skills (e.g. deep, navigation, buoyancy, maybe night or wreck)  
    - Experience profile (types of sites, conditions, depths).  
  - Optionally: monitor or surface options for the **next suitable trip window** (e.g. July in Europe within budget).

---

### Experience focus for Kenya (questions / TODO for later)

- **Kenya details to clarify**
  - Where exactly: Diani, Watamu, Mombasa, other?  
  - How many days of diving / how many dives?  
  - Which AOW adventure dives: deep, navigation, peak buoyancy, night, wreck, etc.?  

- **Learning priorities for Kenya**
  - Use AOW to:  
    - Build **comfort at depth** (deep dive).  
    - Sharpen **navigation** and **buoyancy** (so future wreck/cavern diving is safer and more fun).  
    - Get at least one “different” experience (night or limited‑visibility, or mild current).  

---

### Europe July trip – open questions

- **Timing & constraints**
  - Is July fixed, or a bit flexible?  
  - Rough trip length: still 4–5 days?  
  - Budget: still around **£1000** total?  

- **Fit with post-Kenya profile**
  - With **~15–20 dives and AOW** after Kenya + (possibly) some July dives, what’s the **best value** in Europe for:  
    - Wrecks / mild caverns  
    - Clear, warm-ish water  
    - Reasonable cost and travel time from home base  

---

### Year‑ahead sketch (high level)

- **Kenya (March)** → certification + skills foundation  
- **Europe (July, e.g. Malta)** → first wreck/cavern exposure with new AOW  
- **Late 2025** → one more trip tuned to whatever you enjoyed most (reefs vs wrecks vs adventure)  
- **2026–2027** → repeat the pattern, slowly increasing complexity and variety, staying aligned with 50‑dive goal.

